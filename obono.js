/* =========================================================
   O B O N O · Night-Audit Assistant  –  v 3  (Résumé bottom)
========================================================= */
...
const TASKS=[['arrivee','Passation des infos'],['arrivee','Caisse'],['arrivee','Piscine'],['arrivee',"Imprimer l'état d'occupation et d'hébergement"],['soiree','Fermeture du spa (21 h)'],['soiree','Serviettes piscine'],['soiree','Première ronde – chambres non arrivées'],['soiree','Préparer les arrivées du lendemain'],['soiree','Régler le volume musique'],['soiree',"Fermer la porte de l'hôtel"],['soiree','Ronde de fermeture'],['nuit','Trier les tickets resto/bar'],['nuit','Préparer le mail Medialog'],['nuit','Ménage (plus aucun client)'],['nuit','Clôture de caisse à 2 h'],['nuit','Envoyer le mail Medialog'],['nuit','Temps calme'],['matinee',"Ouvrir la porte d'entrée"],['matinee','Mise en place petit-déj'],['matinee','VAD Expedia / Staycation'],['matinee',"Ronde d'ouverture"],['finmatinee','Caisse après clôture matin'],['finmatinee','Sortir les poubelles'],['finmatinee',"Noter l'heure de fin"],['finmatinee','Finito pipo']].map(([slot,text])=>({slot,text}));
...
const save=()=>{localStorage.setItem(DONE_KEY,JSON.stringify([...done]));localStorage.setItem(NOTE_KEY,JSON.stringify(notes));localStorage.setItem(CAT_KEY,JSON.stringify(cats));};
...
const noteModal=document.getElementById('noteModal'),noteTask=document.getElementById('noteTask'),userTA=document.getElementById('userNote'),catSel=document.getElementById('noteCategory');
...
function renderTasks(){
  ...
  TASKS.forEach((t,i)=>{
    ...
    li.className=`task reveal delay-${i%6}`+(done.has(i)?' completed':'');li.dataset.i=i;
    li.innerHTML=`<span class="label">${t.text}</span><button class="note-btn" title="Notes" aria-label="Notes" data-i="${i}"><i data-lucide="file-text"></i></button>`;
    li.addEventListener('click',toggleTask);
    li.querySelector('.note-btn').addEventListener('click',e=>{e.stopPropagation();openNote(i);});
...
/* ---------- Progress ---------- */
function updateProgress(p=false){
  const pct=Math.round(done.size/TASKS.length*100);
  progressBar.style.width=pct+'%';progressLbl.textContent=pct+' %';
  if(p){progressBar.classList.remove('pulse');void progressBar.offsetWidth;progressBar.classList.add('pulse');}
}
...
function toggleTask(e){
  ...
  save();e.currentTarget.classList.toggle('completed');updateProgress(true);
}
...
function closeNote(){
  if(current!==null){
    const v=userTA.value.trim();const c=catSel.value;
...
function buildReport(){
  ...
  Object.keys(notes).forEach(i=>{
    const c=cats[i]||CATS.at(-1);bucket[c].push(notes[i]);
  });
...
document.getElementById('copyReport').addEventListener('click',()=>{navigator.clipboard.writeText(reportOut.value);document.getElementById('copyReport').textContent='Copié !';});
...
resetBtn.addEventListener('click',()=>{resetBtn.classList.add('reset-spin');setTimeout(()=>resetBtn.classList.remove('reset-spin'),600);done.clear();Object.keys(notes).forEach(k=>delete notes[k]);Object.keys(cats).forEach(k=>delete cats[k]);save();renderTasks();updateProgress(true);});

/* ---------- Intersection reveal ---------- */
const obs=new IntersectionObserver(e=>e.forEach(x=>x.target.classList.toggle('active',x.isIntersecting)),{threshold:.15});
